...
Five steps, each its own CLI subcommand:
...
4. **`review-faces`** — verify detections in a local web UI (cropped face +
   predicted name) before anything gets written to Lightroom.
5. **`write-xmp`** — write the *approved* results back out as XMP face
   regions so Lightroom can show them.
...
- **Step 4** launches a local [Gradio](https://www.gradio.dev/) web app
  (`http://127.0.0.1:7860` by default) where you review detected faces one
  at a time: a padded crop of the face, the predicted name, and its
  confidence. You approve it (optionally correcting the name first, which
  also fixes previously-`unknown` faces), reject it, or skip it for later.
  Faces default to the lowest-confidence ones first, since those are most
  likely to be wrong. Nothing is written anywhere else on disk by this step
  — decisions are recorded as a `review_status` on each face in `faces.db`.
  See [`face_pipeline/review/ui.py`](face_pipeline/review/ui.py).
- **Step 5** writes face regions back out using the Metadata Working Group
  ...
  for proprietary raw formats, matching Lightroom's own convention. **Only
  faces approved in step 4 are written** — anything still pending or
  rejected is skipped. See
...
crops, train, run inference, check status, review faces in the browser,
write XMP back), so you can re-run individual steps as needed. Re-running
`run.bat` later skips the setup steps (env/deps already in place) and goes
...
docker compose -f docker/docker-compose.yml --profile gpu run --rm pipeline-gpu export-status
#    -> total exported faces and a per-person breakdown, straight from the
#       Lightroom export (not run-inference's detections -- see `status` below)
...
# 4. Once happy, run the full library:
docker compose -f docker/docker-compose.yml --profile gpu run --rm pipeline-gpu run-inference

# 5. Review detections in the browser before anything is written (--service-ports
#    publishes the port from docker-compose.yml; the UI needs --host 0.0.0.0 to
#    be reachable from outside the container -- then open http://localhost:7860):
docker compose -f docker/docker-compose.yml --profile gpu run --rm --service-ports pipeline-gpu review-faces --host 0.0.0.0

# 6. Write the approved results back to Lightroom:
...
`review-faces` needs the `review` extra (installs [Gradio](https://www.gradio.dev/)); the
other steps don't.
...
- Nothing in this project ever writes into your Lightroom `.lrcat` catalog —
  only into a separate local `faces.db` and, in step 5, XMP metadata in your
  photo files/sidecars.
- `write-xmp` only writes faces with `review_status = 'approved'`. If you
  re-run `run-inference` on an image that changed (new mtime), its old faces
  — and their review decisions — are replaced, since the classifier's
  predictions for it may now differ; re-review it in `review-faces`.
